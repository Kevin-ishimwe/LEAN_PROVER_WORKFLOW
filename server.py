...
def load_hf(hf_model):
    print("Loading model...")
    model = transformers.AutoModelForCausalLM.from_pretrained(hf_model,low_cpu_mem_usage=True)
...
class RequestHandler(BaseHTTPRequestHandler):
    def process_request(self, tactic_state, prefix):
        prompt = self.server.config['PROMPT'](tactic_state, prefix)
        texts = self.server.generate_function(
            model=self.server.model,
            tokenizer=self.server.tokenizer,
            prompt=prompt,
            temperatures=self.server.config['TEMPERATURES'],
            num_samples=self.server.config['NUM_SAMPLES']
        )
